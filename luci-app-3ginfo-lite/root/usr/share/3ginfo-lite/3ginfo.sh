...
# Use HiLink IP if configured, otherwise use device setting
HILINK_ADDR=""
if [ -n "$HILINK_IP" ]; then
	HILINK_ADDR="$HILINK_IP"
elif echo "x$CONF_DEVICE" | grep -q "192.168."; then
	HILINK_ADDR="$CONF_DEVICE"
fi

if [ -n "$HILINK_ADDR" ]; then
	# Try to detect modem type, with fallback to generic HiLink detection
	MODEM_DETECTED=0
	
	# Try USB detection first
	if grep -q "Vendor=1bbb" /sys/kernel/debug/usb/devices 2>/dev/null; then
		. $RES/modem/hilink/alcatel_hilink.sh $HILINK_ADDR
		MODEM_DETECTED=1
	elif grep -q "Vendor=12d1" /sys/kernel/debug/usb/devices 2>/dev/null; then
		. $RES/modem/hilink/huawei_hilink.sh $HILINK_ADDR
		MODEM_DETECTED=1
	elif grep -q "Vendor=19d2" /sys/kernel/debug/usb/devices 2>/dev/null; then
		. $RES/modem/hilink/zte.sh $HILINK_ADDR
		MODEM_DETECTED=1
	fi
	
	# If no USB detection, try to ping and detect via web interface
	if [ $MODEM_DETECTED -eq 0 ]; then
		# Try to detect by querying the web interface
		WEBTEST=$(wget -t 2 -T 3 -q -O - "http://$HILINK_ADDR/api/device/information" 2>/dev/null)
		if echo "$WEBTEST" | grep -q "Huawei\|huawei"; then
			. $RES/modem/hilink/huawei_hilink.sh $HILINK_ADDR
			MODEM_DETECTED=1
		elif echo "$WEBTEST" | grep -q "ZTE\|zte"; then
			. $RES/modem/hilink/zte.sh $HILINK_ADDR
			MODEM_DETECTED=1
		elif echo "$WEBTEST" | grep -q "Alcatel\|alcatel"; then
			. $RES/modem/hilink/alcatel_hilink.sh $HILINK_ADDR
			MODEM_DETECTED=1
		else
			# Default to Huawei for B312-929 and similar modems with mac-vlan
			. $RES/modem/hilink/huawei_hilink.sh $HILINK_ADDR
			MODEM_DETECTED=1
		fi
	fi
